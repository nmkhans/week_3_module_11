...
def home(req): 
  return render(req, "meals/index.html", {'meals': meals})

def mealDetail(req, id):
  def check(meal):
    if meal['idMeal'] == str(id):
      return True
    else:
      return False
  
  meal = list(filter(check, meals))[0]
  print(meal)
  return render(req, "meals/mealsDetail.html", {'meal': meal})